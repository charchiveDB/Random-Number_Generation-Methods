...
chi_squared_test_norm <- function(x, k){
  ...
  bins <- qnorm(seq(0, 1, length.out = k+1), mean = 0, sd = 1) #200 bins with equal prob
...
```{r, echo=FALSE}
hist(x_2, breaks = 100, freq = FALSE, main="Distribution of Normal Samples", xlab="x")
...
chi_squared_test_exp <- function(x, k){
  t <- length(x) # number of observations
  #binomial distribution approximately normal when np > 5, when we use bins with equal width np gets     very small which means Y_i is not approx. normal and the chi square test fails b/c test stat not       normal
  bins <- qexp(seq(0, 1, length.out = k+1), rate = 1) #200 bins all with the same probability

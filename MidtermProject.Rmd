...
```{r plot, echo=FALSE}
hist(x, breaks = 100, freq=FALSE, main="Distribution of Normal Samples")
...
To test formally, we apply 2 different types of tests: The Kolmogorov-Smirnov Test and the Chi-Squared test.

With the Kolmogorov-Smirnov test we expect to not have sufficient evidence to reject $H_0$, that being

$$
H_0 : f_x(x) = \frac{1}{\sqrt{2\pi}}exp(\frac{x^2}{2})
$$

```{r k-s}
ks.test(x, "pnorm", mean=0, sd=1) # passed
```

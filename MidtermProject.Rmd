...
```{r plot, echo=FALSE, out.width="50%", fig.align="center"}
hist(x_1, breaks = 100, freq=FALSE, main="Distribution of Normal Samples", xlab="x")
curve(dnorm(x), add=TRUE, col="purple", lwd=2) # looks good
```
...
$p_i =$ probability that each observation falls into the the category i, $\frac{1}{k}$ in our implementation

$Y_i =$ the number of observation that actually do fall into the category i

Using `rnormzig()` to generate 1,000,000 random variates, the distribution generated was tested based on 200 bins of equal density such that values have a less than 1 in 1 billion chance of generating from outside out bins. The critical value for a chi-square test with 199 degrees of freedom at 95% confidence level ($\alpha= 0.05$) is 232.912. 
...
Take a sample from `rnorm()` and compare it to our sample that we drew previously using the Ziggurat method.
...
combined$p_value <- format.pval(combined$p_value, digits = 3)
kable(combined, caption = "Chi-Squared Test: Ziggurat vs rnorm",
      col.names = c("Method", "$\\chi^2$", "df", "p"),
      escape = FALSE,
      booktabs=TRUE)
```
...
ggplot(df, aes(x = n, y = chi_square)) +
  ...
  geom_point() +
  scale_x_log10() +
  geom_vline(xintercept = 232.912, linetype = "dashed", color = "red") +
  annotate("text", x = 232.912, y = Inf, label = "Critical Value", 
...
In the plot above, it is clear than varying numbers of samples all have a chi-square value less than the critical value of 232.912. Thus, we have evidence to suggest that we successfully generated random variables that follow a normal distribution.
...
curve(dexp(x), add=TRUE, col="purple", lwd=2) # looks good
```

Repeating the steps from the previous section, we can write a new function. The main difference is that the exponential distribution starts from 0 instead of $-\infty$ and the $x$ value that ensures less than 1 in 1 billion samples generate outside our bins is different from the normal.
...
#Format for readability 
combined$p_value <- format.pval(combined$p_value, digits = 3)
kable(combined, caption = "Chi-Squared Test: Ziggurat vs rexp",
      col.names = c("Method", "$\\chi^2$", "df", "p"),
      escape = FALSE,
      booktabs=TRUE)
```
...
Again, all sample sizes show $\chi^2$ statistics well below our critical value of 232.912, so we have evidence that we successfully generated from the exponential distribution.
...
n_values <- seq(from = 1000, to = 100000, length.out = 15)
# vectors to store results
times_rnorm <- numeric(length(n_values))
times_zig   <- numeric(length(n_values))
...
for (i in seq_along(n_values)) {
  ...
  # rnorm
  times_rnorm[i] <- system.time({
    rnorm(n)
  })[["elapsed"]]
  
  # rnormzig
  times_zig[i] <- system.time({
    rnormzig(n)
  })[["elapsed"]]
}
